...
<template>
    <div class="lg:px-4 px-2 py-2 sticky top-0 z-50 w-full flex flex-col space-y-2 BlurFill bg-opacity-50">
        ...
        >
            ...
            <!-- Visible on Desktop -->
            <div 
             class="hidden md:flex space-x-6 uppercase font-TW"
            >
                <div
                 :class="darkMode.isDark ? 'hover:text-indigo-500':'hover:text-green-500'"
            ...
            <button 
                @click="darkMode.isDark = !darkMode.isDark" 
                class="rounded-md px-2 py-2"
                :class="darkMode.isDark ? 'bg-darkMode':'bg-purple-900'"
            >
                <div v-show="darkMode.isDark === false" class="transition-all duration-75">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
                    </svg>
                </div>
                <div v-show="darkMode.isDark === true"  class="transition-all duration-75">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div 
        v-show="show"
        :class="darkMode.isDark ? 'bg-innerDark':'bg-purple-800' " 
        class="w-full py-4 px-8 md:hidden flex-col space-y-2 rounded-md 
        bg-opacity-80 BlurFill justify-center items-center uppercase font-medium"
        v-motion
